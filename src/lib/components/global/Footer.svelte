<script lang="ts">
  ...
  import { onMount } from "svelte";
    ...
    onMount(()=>{
        ...
        links.forEach((link: { addEventListener: (arg0: string, arg1: { (event: any): void; (event: any): void; }) => void; }) => {
...
<footer class="bg-white px-10 pt-20 pb-4">
    <section class="flex flex-col md:flex-row space-y-8 md:space-y-0 justify-between">
        <div class="flex flex-col items-center md:items-start space-y-4">
            <a class="footer-links transition-all text-black text-xl" href="/" data-sveltekit-preload-data="tap">home</a>
            <a class="footer-links transition-all text-black text-xl" href="/work" data-sveltekit-preload-data="tap">work</a>
            <a class="footer-links transition-all text-black text-xl" href="/about" data-sveltekit-preload-data="tap">about</a>
            <a class="footer-links transition-all text-black text-xl" href="/blogs" data-sveltekit-preload-data="tap">blog</a>
            <a class="footer-links transition-all text-black text-xl" href="/contact" data-sveltekit-preload-data="tap">contact us</a>
        </div>
        <div class="flex flex-col items-center space-y-8 md:space-y-4 md:items-end justify-between">
            ...
            <div class="subsription-wrapper" bind:this={subscriptonWrapper}>
                ...
                {:else}
                    <div class="flex flex-col md:flex-row items-center justify-center space-y-3 md:space-y-0 md:space-x-3">
                            <input class="bg-white border border-black text-black text-center text-md h-8 w-56" type="text" placeholder="your email address" bind:value={user_email}>
                            <button class="bg-black h-8 w-56" type="submit" on:click={userSubscribe}>Subscribe to our mailing</button>
                    </div>
                {/if}
            </div>
        </div>
    ...
    <div class="flex flex-col justify-center">
        <div class="w-full h-px bg-black mt-10 mb-2"></div>   
        <a href="/policy" class="text-black text-center md:text-start text-md">Privacy & Cookie Statement</a>
    </div>
</footer>