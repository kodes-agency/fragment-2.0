<script lang="ts">
  ...
  console.log($page.data.data?.projectCategories?.data);
  // @ts-ignore
...
//       if ($page.route.id == "/work/work-more") {
//         gsap.to(".work-button", {
//           duration: 1,
//           opacity: 1,
//           stagger: 0.1,
...
<section id="work" class=" relative min-h-[130vh] p-5 py-20 md:p-20 lg:p-40 flex flex-col items-center" bind:this={section}>
    <div class="flex flex-col space-y-3 pb-10">
      <h2 class="gradient gradient-anm gradient-text text-center font-bold text-5xl lg:text-6xl" id="work-h2">
        {sectionData?.workHeading}
      </h2>
      <div class="overflow-hidden">
        <h4 id="work-h4" class="text-center text-3xl">{sectionData?.workSubtext}</h4>
      </div>
    </div>
    <div class="flex items-center md:justify-center md:space-x-5 flex-wrap pb-16 justify-start">
      <button
    ...
    <div class="projects-wrapper">
      {#each projects as project}
        <div
          ...
          data-category={project.attributes.category?.data?.attributes
            ?.category}
        >
          <ProjectItem
            id={project.id}
