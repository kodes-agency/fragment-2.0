<script lang="ts">
  ...
  $: displayBlogs = blogs
    .filter((blog) => {
      return blog.attributes.slug != $page.params.slug;
    })
...
<section
  class="flex flex-col items-center justify-center p-7 py-20 md:p-20 lg:p-40 space-y-12"
>
  <h2
    class="center font-bold text-4xl md:text-5xl gradient gradient-anm gradient-text py-5"
  >
  ...
  <div class="gradient gradient-anm">
    <a
      class="py-1 px-10 border border-white h-full w-full bg-black hover:bg-[rgba(0,0,0,0.2)]"
      href="{$page.params.lang
        ? '/' + $page.params.lang
        : ''}/blog?repeat=true#collection"
      >{$page.data.layoutData?.data?.menu?.data?.attributes?.viewAllBlogs}</a
    >
  </div>
</section>
